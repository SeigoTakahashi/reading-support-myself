...
export default function LoginPage() {
  // AuthContextから関数を取得
  const { login, googleLogin } = AuthContextConsumer();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setAuthError({
        title: "ログインできませんでした",
        // 初心者向けにわかりやすい説明（英語の生メッセージは出さない）
        detail:
          "メールアドレスかパスワードが間違っている可能性があります。入力を確かめてもう一度お試しください。",
      });
    } finally {
      setIsLoading(false);
    }
        ...
        <div className="relative w-full max-w-md">
          ...
          <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl shadow-indigo-500/10 p-8 border border-white/20 animate-fadeIn">
            ...
            <div className="space-y-6">
              ...
              {/* ログインボタン */}
              <button
                className="w-full bg-linear-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white py-3 px-4 rounded-xl font-medium shadow-lg hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500 transition-all duration-200 transform hover:scale-[1.02]"
